...
class Extractor():
    ...
    def loadMeta(self, file):
        with open(file) as fp:
            data = json.load(fp)
        return data

    def startJob(self, client, bucket, file, type):
        response = None
        if type == 'Analyze':
    ...
    def isJobComplete(self, client, jobId, type):
        status = 'IN_PROGRESS'
    ...
    def getJobResults(self, client, jobId, type):
        if self.isJobComplete(client, jobId, type):
            if type == 'Analyze':
                response = client.get_document_analysis(JobId=jobId)
            elif type == 'Detect':
                response = client.get_document_text_detection(JobId=jobId)
    ...
    # contains
    def getText(self, token, blocks_map):
        words = []
    ...
    def getRemainder(self, tokens, nas, nlp):
        global META
        token_map = {}
        line_ids = []
    ...
    def getMapping(self, token_map, key_ids):
        mappings = {}

        for key_id in key_ids:
            key = token_map[key_id]
            value_ids = self.getValueIds(key)
            values = []
            for value_id in value_ids:
                value = token_map[value_id]
                values.append(self.getText(value, token_map))
            value_text = ' '.join(values)
            key_text = self.getText(token_map[key_id], token_map)
            mappings.update([(key_text, value_text)])
        return mappings

    def getKeyValues(self, tokens, META):
        score_list = {}
    ...
    def findStudentInfo(self, tokens):
        token_map = {}
        line_ids = []
        ...
        for token in tokens:
            ...
            if token['BlockType'] == 'LINE' and 'Geometry' in token:
                if all(word in token['Text'] for word in info_words):
                    ...
                    line_ids.append(token_id)
        if stu_section != None:
            min, max = self.getMinMax(stu_section)
